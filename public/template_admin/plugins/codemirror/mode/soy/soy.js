        ...
        switch (last(state.soyState)) {
          ...
          case "templ-ref":
            if (match = stream.match(/(\.?[a-zA-Z_][a-zA-Z_0-9]+)+/)) {
              state.soyState.pop();
              // If the first character is '.', it can only be a local template_admin.
              if (match[0][0] == '.') {
                return "variable-2"
              }
              // Otherwise
          ...
          case "param-type":
            ...
            if ("}]=>,".indexOf(peekChar) != -1) {
              ...
              state.soyState.push('param-type-template_admin');
              return null;
            } else if (peekChar == "<") {
          ...
          case "param-type-template_admin":
            if (stream.match(/[>]/)) {
              state.soyState.pop();
              state.soyState.push('param-type');
              return null;
          ...
          case "tag":
            ...
            if (stream.match(/^\/?}/)) {
              ...
              if (state.tag == "/template_admin" || state.tag == "/deltemplate") {
